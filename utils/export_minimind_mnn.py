...
shutil.copy(
    os.path.join(src_model, "model_vlm.py"), os.path.join(HF_DIR, "modeling_vlm.py")
)
shutil.copy(
    os.path.join(src_model, "model_minimind.py"),
...
shutil.copy(
    os.path.join(src_model, "tokenizer.json"), os.path.join(HF_DIR, "tokenizer.json")
)
shutil.copy(
    os.path.join(src_model, "tokenizer_config.json"),
...
cmd = (
    "{} -f ONNX --modelFile {} --MNNModel {} --transformerFuse --allowCustomOp".format(
        MNNCONVERT, embedded_onnx, base_mnn
    )
)
            ...
            new_ops.append(
                {
                    ...
                    "main": {
                        ...
                        "kv_shared_layer_index": kv_shared,
                    },
                    "name": name,
...
cmd = "{} -f JSON --modelFile {} --MNNModel {}".format(
    MNNCONVERT, fixed_json, final_mnn
)
os.system(cmd + " 2>/dev/null")
