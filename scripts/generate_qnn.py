...
def seperate(args, model_name, ids):
    ...
    with open(os.path.join(cache, 'qnn.json'), 'w') as f:
        f.write(json.dumps(config, indent=4))
    process = subprocess.Popen(exe + ' ' + model + ' qnn/' + model_name + ' qnn.json', bufsize=1, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd = cache, universal_newlines=True, shell=True)
...
def compile_qnn(args):
    # Keep compatibility helpers in the application project so the MNN
    # submodule remains untouched.
    exe = os.path.join(os.path.dirname(os.path.abspath(__file__)), "npu_convert.py")
    cache = os.path.join(os.getcwd(), args.cache_path)
    process = subprocess.Popen(sys.executable + " " + exe + ' npu_postreat.json %d ' %args.soc_id + ' ' + args.dsp_arch, bufsize=1, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd = cache, universal_newlines=True, shell=True)
    for line in process.stdout:
        print(line, end='')
    process.wait()
    return process.returncode
...
def output_qnn(args):
    ...
    if args.need_config_json is True:
        ...
        if (args.model_name == "llm.mnn" and
                not os.path.exists(config_path)):
            config_path = os.path.join(args.model, 'llm_config.json')
        config_npu = {}
        ...
        if not is_visual:
            config_npu["llm_config"] = "llm_config_qnn.json"
            config_npu["llm_model"] = "qnn/llm.mnn"
            config_npu["llm_weight"] = "qnn/llm.mnn.weight"
...
def convert_qnn(args, model_name, inputjson, external_file, ids):
    ...
    if seperate(args, model_name, ids) != 0:
        raise RuntimeError("MNN QNN graph separation failed")
    end = time.time()
    print("Cost: ", end - sta, ' s')
    sta = end
    print("Step3: Compile to QNN")
    if compile_qnn(args) != 0:
        raise RuntimeError("QNN context binary compilation failed")
    end = time.time()
    print("Cost: ", end - sta, ' s')
